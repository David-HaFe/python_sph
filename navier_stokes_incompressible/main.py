# main file for simulating incompressible fluid with navier stokes equations
# author: David Hambach Ferrer

import numpy as np
import random as rnd

from solvers.chorin import chorin
from solvers.euler_forward import euler_forward
import navier_stokes_incompressible.dynamics as navier_stokes_incompressible
import navier_stokes_incompressible.pressure_poisson as pressure_poisson
from utils.diagnostics import diagnostics
from utils.generate_border import generate_border

from config import (
    no_particles_x,
    no_particles_y,
    no_particles,
    x_positions,
    y_positions,
    border,
    t0,
    t1,
    no_steps,
    dt,
)


def main():

    def noise():
        return -0.1 + 0.2 * rnd.random()

    r_0 = []
    v_0 = []
    p_0 = []
    is_border_particle = []

    for _, y in enumerate(y_positions):
        for _, x in enumerate(x_positions):
            r_0.extend([x, y])

            x_vel = y  # proportional to y coordinate, resulting in a shear

            v_0.extend([x_vel, 0])
            p_0.extend([1])

            # add wall particle flag
            is_border_particle.extend([False])

    r_0, v_0, p_0, is_border_particle = generate_border(
        r_0, v_0, p_0, is_border_particle
    )

    r_0 = np.array(r_0, dtype=float)
    v_0 = np.array(v_0, dtype=float)
    p_0 = np.array(p_0, dtype=float)
    y_0 = np.concatenate((r_0, v_0, p_0))
    is_border_particle = np.array(is_border_particle)

    # simulation
    diagnostics.time_ode()

    # sol = euler_forward(
    #     function=navier_stokes,
    #     initial_condition=y_0,
    #     t_start=0,
    #     t_end=4,
    #     dt=.01,
    # )

    # solver setup
    t_span = (t0, t1)
    t_eval = np.linspace(t0, t1, num=no_steps)

    sol = chorin(
        forward_equation=lambda t, y: navier_stokes_incompressible.dynamics(
            t,
            y,
            is_border_particle,
        ),
        projection_equation=lambda t, y: pressure_poisson.equation(
            t,
            y,
            dt,
            is_border_particle,
        ),
        initial_condition=y_0,
        t_start=t0,
        t_end=t1,
        dt=0.01,
    )

    # sol = solve_ivp(
    #     fun=lambda t, y: navier_stokes_compressible(
    #         t,
    #         y,
    #         is_wall_particle,
    #     ),
    #     t_span = t_span,
    #     y0 = y_0,
    #     method = "RK23",
    #     rtol = 1e-3,
    #     atol = 1e-3,
    #     t_eval = t_eval,
    # )
    # print("")
    # print(sol.message)

    diagnostics.time_ode()

    x = sol.y[0 : 2 * no_particles : 2, :]
    y = sol.y[1 : 2 * no_particles : 2, :]

    t = sol.t

    return t, x, y, is_border_particle
